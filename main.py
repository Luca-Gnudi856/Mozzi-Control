...
def connect_modbus():
    while True:
        try:
            print("Attempting to connect to Modbus...")
            md.Connect()
            if md.c.is_open:
...
def reconnect():
    """Function to reconnect in case of Modbus disconnection."""
    md.c.close()  # Close any existing connection
    connect_modbus()  # Attempt to reconnect
...
def readLight():
    return md.ReadSingleHoldingRegister(512)
...
def read_settings():
    max_retries = 20  # Maximum number of retries if the file is locked
    ...
    while retries < max_retries:
        try:
            ...
            shour, sminute = map(int, timeOn.split(":"))
            Shexhour = "0x"+str(shour)
            Snewhour = int(Shexhour, 16)
            Shexminute = "0x"+str(sminute)
            Snewminute = int(Shexminute, 16)
            StartTime = (Snewhour << 8) | Snewminute  # Combine hour and minute into 16-bit word

            hour, minute = map(int, timeOff.split(":"))
            hexhour = "0x"+str(hour)
            newhour = int(hexhour, 16)
            hexminute = "0x"+str(minute)
            newminute = int(hexminute, 16)
            EndTime = (newhour << 8) | newminute  # Combine hour and minute into 16-bit word
...
def read_control():
    ...
    while retries < max_retries:
        try:
            ...
            # Parse the settings as usual
            temp1, humid1, light1, dayLight1, sample1, mode1, count1, timeOn, timeOff, mosquitoMode = [
                line.strip().split(',')[0] for line in lines[:10]
            ]
            ...
            timeOn = timeOn.split("=")[1]
            timeOff = timeOff.split("=")[1]
            mosquitoMode = mosquitoMode.split("=")[1]
            ...
            return temp1, humid1, light1, dayLight1, sample1, mode1, count1, timeOn, timeOff, StartTime, EndTime, mosquitoMode

        except (OSError, IOError) as e:
            # If the file is locked or access is denied, handle the exception
            print(f"Could not access {control_file} (attempt {retries + 1}/{max_retries}). Retrying in {retry_delay} seconds...")
            retries += 1
            time.sleep(retry_delay)

    # If the maximum retries are exceeded, raise an error or return a fallback value
    raise TimeoutError(f"Could not access {control_file} after {max_retries} retries.")
...
last_mod_time_2 = os.path.getmtime(control_file)
...
try:
    while True:
        ...
        # Initialize previous values (no change needed)
        
        try:
            # Open CSV file and read initial settings (no change needed)
            while True:
                ...
                temp1, humid1, light1, dayLight1, sample1, mode1, count1, timeOn, timeOff, StartTime, EndTime, mosquitoMode = read_control()
                ...
                while md.c.is_open:  # Continue the loop while the Modbus connection is open
                    ...
                    current_mod_time_2 = os.path.getmtime(control_file)
                    
                    if current_mod_time_2 != last_mod_time_2:
                        # If modified, read the new controls
                        print("Detected changes in control.txt, updating controls.")
                        temp1, humid1, light1, dayLight1, sample1, mode1, count1, timeOn, timeOff, StartTime, EndTime, mosquitoMode = read_control()
                    
                    if mosquitoMode == "true":
                        ...
                        last_mod_time_2 = current_mod_time_2
                    
                    elif mosquitoMode == "false":
                ...
                reconnect()  # Try reconnecting if there was an error   
